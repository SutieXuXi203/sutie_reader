...
const UserTableRow = memo(function UserTableRow({
    ...
    return (
        <tr className="group hover:bg-secondary/70 dark:hover:bg-primary/10 transition-colors">
            <td className="px-5 py-4 text-left border-r border-border/40 last:border-r-0">
                <div className="flex items-center justify-start gap-3">
                    ...
                    <p className="text-sm font-medium text-foreground truncate max-w-[150px]">{user.name || 'Ẩn danh'}</p>
                </div>
            </td>
            <td className="px-5 py-4 text-sm text-foreground/90 text-left border-r border-border/40 last:border-r-0">{user.email}</td>
...
const UserMobileCard = memo(function UserMobileCard({
    ...
    return (
        <article className="rounded-[8px] border border-border/70 bg-card/55 p-3 shadow-sm">
            <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-semibold text-foreground">{user.name || 'Ẩn danh'}</p>
            ...
            <div className="mt-3 grid grid-cols-2 gap-2 text-[11px] text-muted-foreground">
                ...
                <div className="text-right">
                    <p>Ngày tham gia</p>
                    <p className="mt-0.5 font-medium text-foreground/80">{new Date(user.createdAt).toLocaleDateString('vi-VN', { day: '2-digit', month: '2-digit', year: 'numeric' })}</p>
                </div>
            </div>
            <button
                type="button"
...
export const UsersTab = memo(
    function UsersTab({
        ...
        return (
            <div className={isActive ? 'w-full min-h-[360px] lg:min-h-[680px] space-y-3 sm:space-y-4 pb-4' : 'hidden'} aria-hidden={!isActive}>
                {/* Active Users Table */}
                        ...
                        {isUsersLoading ? (
                            ...
                            paginatedUsers.map((u) => (
                                <UserMobileCard
                                    ...
                                    isUpdatingRole={updatingRoleId === u._id}
    ...
    (prev, next) => {
        ...
        return (
            ...
            prev.usersList === next.usersList &&
            prev.isUsersLoading === next.isUsersLoading &&
            prev.usersLoadError === next.usersLoadError &&
            prev.updatingRoleId === next.updatingRoleId &&
            prev.deletedAccounts === next.deletedAccounts &&
            prev.isDeletedAccountsLoading === next.isDeletedAccountsLoading &&
            prev.searchQuery === next.searchQuery &&
            prev.onChangeRole === next.onChangeRole &&
            prev.onDeleteUser === next.onDeleteUser &&
            prev.onRefreshUsers === next.onRefreshUsers &&
            prev.onRefreshDeletedAccounts === next.onRefreshDeletedAccounts
        );
    }
);
